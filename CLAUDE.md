...
**Agent service** (`services/agent_service/`): the chat Q&A LangGraph workflow with nodes: classify_and_route -> executor -> evaluate_batch -> generate, plus out_of_scope and confirm_ingest (HITL). Tools (in `tools/`): retrieve_chunks, arxiv_search, ingest_papers, propose_ingest, list_papers, explore_citations, semantic_scholar. SSE streaming with custom event types (STATUS, CONTENT, SOURCES, CITATIONS, INGEST_COMPLETE, DONE). **Paper-scoped chat** (SPE-277): `POST /stream` accepts `arxiv_id`; the scope is persisted on `conversations.paper_id` (migration 021) and wins on follow-ups and resumes (`routers/stream.py::resolve_scoped_paper`; 409 `PAPER_NOT_INGESTED` / `SCOPE_MISMATCH`). A scoped `AgentContext(scoped_paper=ScopedPaper)` registers only retrieve_chunks (via `SearchService.retrieve_within_paper`, no RRF threshold), explore_citations and semantic_scholar (`CORPUS_ONLY_TOOLS` hidden), and adds a scope note to the routing and answer prompts. `GET /conversations?arxiv_id=` lists a paper's threads.
...
React 19 + TypeScript + Vite. Zustand stores (chat, settings, sidebar, user) for UI state; TanStack Query v5 for server state (`src/api/*.ts`, one module per domain with key factories + hooks). SSE streaming via `@microsoft/fetch-event-source`. React Router v7 with protected routes (`/feed`, `/chat`, `/library`, `/settings`). Clerk auth. Tailwind CSS v4 (light-only warm stone theme, tokens in `src/index.css`). Markdown rendering with KaTeX math support. Feed pivot UI: `src/pages/FeedPage.tsx` + `src/components/feed/*` (cards, verdict line, score badge, signal chips, Save / Dismiss / Implementing with optimistic cache updates in `src/api/paperState.ts`); `src/pages/PaperDetailPage.tsx` + `src/components/paper/*` (per-dimension breakdown with level distribution, confidence, judgments and quoted evidence; `src/api/scores.ts` polls the 202 response every 5 s for up to 3 min); `src/pages/OnboardingPage.tsx` + `src/components/onboarding/*` (categories, compute profile, keywords; `OnboardingGate` in `App.tsx` redirects a signed-in user with `me.onboarded === false` to `/onboarding`, outside the sidebar layout; the same form is editable under Settings); `src/components/paper/ScopedChatPanel.tsx` (paper detail chat: `useChat(sessionId, { arxivId, onSessionCreated })` keeps a scoped draft key and skips the `/chat/:id` redirect; the global `useChatStore` allows one mounted chat surface at a time, so the panel aborts on unmount; `?session=` selects a thread). Tests live under `frontend/tests/**` (vitest + jsdom).

### Database

PostgreSQL 16 + pgvector. Migrations via Alembic (`backend/alembic/`). Tables: papers, chunks, conversations, conversation_turns, users, agent_executions, task_executions, usage_counters, plus the feed-pivot scoring tables paper_scores, score_evidence, user_paper_states, digests (migrations `019_add_scoring_tables`, `020_add_score_dimensions`, `021_add_conversation_paper_id`; see `docs/design/scoring-pipeline.md`).
