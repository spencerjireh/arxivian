  ...
  if (task_type && task_type.answer !== 'other') {
    chips.push(
      <Chip
        ...
        title={`${Math.round(task_type.confidence * 100)}% confidence`}
      >
  ...
  if (model_family && model_family.answer !== 'other') {
    chips.push(
      <Chip
        key="family"
        size="md"
        icon={<Layers className={iconClass} strokeWidth={1.5} />}
        title={`${Math.round(model_family.confidence * 100)}% confidence`}
      >
