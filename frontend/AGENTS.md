...
openapi.json                 the backend's OpenAPI document, written by `just types`; CI fails on drift
src/types/api.gen.ts         generated by openapi-typescript (scripts/generate-api-types.mjs); never edit
src/types/api.ts             named aliases over api.gen.ts (`Schemas['FeedItem']` etc.) plus client-only types (Message, PaperScoreResult, FeedParams)
...
API types are generated: a backend schema change is `just types` (export the document,
regenerate `api.gen.ts`), then `tsc` lists every client site the change touches. Response
fields are required (the backend's `ResponseModel` base), request fields keep their
optionality, the SSE event models come from the stream route's documented 200, and
`ErrorResponse` is the default response on every route.
