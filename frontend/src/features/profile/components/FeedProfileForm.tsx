...
export default function FeedProfileForm({
  initialValue,
  onSubmit,
  isSubmitting,
  submitLabel,
}: FeedProfileFormProps) {
  ...
  return (
    <form onSubmit={submit} className="space-y-8" noValidate>
      <section>
        <h2 className="font-display mb-1 text-lg text-stone-900">Categories</h2>
        <p className="mb-3 text-sm text-stone-500">
          Which arXiv listings should the weekly digest draw from?
        </p>
        <CategoryPicker
          value={categories}
          onChange={update(setCategories)}
          error={attempted ? errors.categories : undefined}
        />
      </section>

      <section>
        <h2 className="font-display mb-1 text-lg text-stone-900">Compute</h2>
        <p className="mb-3 text-sm text-stone-500">Papers that fit your hardware rank first.</p>
        <ComputeProfilePicker
          value={computeProfile}
          onChange={update(setComputeProfile)}
          error={attempted ? errors.compute_profile : undefined}
        />
      </section>

      <section>
        <h2 className="font-display mb-1 text-lg text-stone-900">
          Keywords <span className="font-body text-sm text-stone-400">(optional)</span>
        </h2>
        <p className="mb-3 text-sm text-stone-500">
          Topics you care about break ties in the ranking.
        </p>
