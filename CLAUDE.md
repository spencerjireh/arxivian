...
- **Compose env wiring:** only variables listed under a service's `environment:` reach that container. Settings are validated at import (`celery_app.py`), so `celery-beat` carries `CLERK_DOMAIN` (the one field without a default) plus the five schedule crons; the feed knobs (`TRIAGE_*`, `ONDEMAND_SCORE_LOCK_SECONDS`) are on `app` and `celery-worker` with `config.py` defaults.
