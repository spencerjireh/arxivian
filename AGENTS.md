...
just types                        # export the OpenAPI document and regenerate frontend/src/types/api.gen.ts
...
backend/
  ...
  src/schemas/base.py          ResponseModel: every response schema inherits it so the OpenAPI document marks its fields required
  scripts/export_openapi.py    writes the OpenAPI document (just types); CI diffs it against frontend/openapi.json
...
frontend/                      see frontend/AGENTS.md for the full map and behavior
  ...
  openapi.json                 the backend's exported OpenAPI document (just types); CI fails on drift
  src/types/api.gen.ts         generated by openapi-typescript from openapi.json; never edit
...
lock check, ruff, ty, OpenAPI export diff), Backend unit + api (coverage gate), Backend
integration (pgvector service, migration round trip), Frontend lint (generated types diff,
eslint + knip, prettier, tsc), Frontend tests (vitest thresholds), Docker images + coolify compose (the only check that exercises
