...
function renderForm(initialValue?: Parameters<typeof FeedProfileForm>[0]['initialValue']) {
  const onSubmit = vi.fn()
  render(<FeedProfileForm initialValue={initialValue} onSubmit={onSubmit} isSubmitting={false} submitLabel="Build my feed" />)
  return onSubmit
}
...
describe('FeedProfileForm', () => {
  it('blocks submit until a category and a compute profile are chosen', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Build my feed' }))
    expect(onSubmit).toHaveBeenCalledWith({ categories: ['cs.LG'], compute_profile: 'single_gpu', keywords: [] })
  })

  it('pre-fills from the initial value and toggles categories off', () => {
    const onSubmit = renderForm({ categories: ['cs.CV', 'cs.LG'], compute_profile: 'cloud', keywords: ['rag'] })
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Build my feed' }))
    expect(onSubmit).toHaveBeenCalledWith({ categories: ['cs.LG'], compute_profile: 'cloud', keywords: ['rag'] })
  })
